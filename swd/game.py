...
class Game:
    ...
    def apply_action(state: GameState, action: Action):
        ...
        if isinstance(action, BuyCardAction):
            ...
            Game.apply_immediate_bonus(state, player_state.index, token.immediate_bonus, True)
    ...
    def add_card(state: GameState, player_state: PlayerState, card: Card):
        ...
        Game.apply_immediate_bonus(state, player_state.index, card.immediate_bonus, True)
    ...
    def apply_immediate_bonus(state: GameState, player_index: int, immediate_bonus: Dict[ImmediateBonus, int],
        ...
        player_state = state.players_state[player_index]
        opponent_state = state.players_state[1 - player_index]
        for bonus, value in immediate_bonus.items():
            if bonus == ImmediateBonus.COINS:
                player_state.coins += value
            elif bonus == ImmediateBonus.SHIELD:
